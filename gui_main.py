...
class GalaxyMsbtEditor(QMainWindow):
    ...
    def init_adapter(self):
        ...
        # Populate selection boxes with data from config
    ...
    def init_events(self):
        ...
        self.buttonMessagesAdd.clicked.connect(self.create_message_entry)
        self.buttonMessagesRemove.clicked.connect(self.remove_message_entry)
        self.buttonMessagesDuplicate.clicked.connect(self.duplicate_message_entry)
        self.buttonMessagesSort.clicked.connect(self.sort_message_entries)

        # Message entry component events
    ...
    def set_msbt_file_components_enabled(self, state: bool):
        ...
        self.buttonMsbtNew.setEnabled(state)
        self.buttonMsbtDelete.setEnabled(state)
        self.listMsbtFiles.setEnabled(state)

    def set_message_entries_components_enabled(self, state: bool):
    ...
    def reset_message_entries_model(self):
        self.message_entry_names.removeRows(0, self.message_entry_names.rowCount())
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Message entries creation, deletion, etc.
    # ------------------------------------------------------------------------------------------------------------------
    def create_message_entry(self):
        ...
        start_row = self.message_entry_names.rowCount()
        self.message_entry_names.insertRow(start_row)
        self.message_entry_names.setData(self.message_entry_names.index(start_row), message_label, 0)
    ...
    def on_message_entry_selected(self):
        ...
        message_label: str = self.message_entry_names.data(selection.indexes()[0], 0)
